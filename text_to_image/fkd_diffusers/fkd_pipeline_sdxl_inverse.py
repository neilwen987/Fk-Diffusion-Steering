...
class FKDStableDiffusionXL(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # 如果提供了 latents（从图像编码得到），需要给它加噪到对应的时间步
        if latents is not None and start_timesteps > 0:
            # 获取对应时间步的噪声水平
            timestep = timesteps[start_timesteps]
            # 添加噪声到 latents
            noise = torch.randn_like(latents)
            latents = self.scheduler.add_noise(latents, noise, timestep)
            print(f'Added noise to latents at timestep {timestep}')
        elif latents is None:
            # 如果没有提供 latents，生成随机噪声
    ...
    def partial_denoise(
        ...
        **kwargs,
        ...
        timesteps = timesteps[start_timesteps:end_timesteps]
...
if __name__ == '__main__':
    ...
    # Load the pipeline
    pipe = FKDStableDiffusionXL.from_pretrained("stabilityai/stable-diffusion-xl-base-1.0",torch_dtype=torch.float32)
    pipe.scheduler = DDIMScheduler.from_config(pipe.scheduler.config)
    pipe.inv_scheduler = DDIMInverseScheduler.from_pretrained("stabilityai/stable-diffusion-xl-base-1.0", subfolder='scheduler')
    pipe.to("cuda")

    # DONE: Test partial denoising
    # start_timesteps =0
    # end_timesteps = 100
    # # Run the pipeline
    # output = pipe.partial_denoise(
    #         'a photo of iron man eatting a burger',
    ...
    # Test partial denoising with condition latents
    start_timesteps =20
    end_timesteps = 100
    # Run the pipeline
    imgname = '/home/ubuntu/tiansheng/Fk-Diffusion-Steering/final_result/test_sdxl_partital_image_20.png'
    img = Image.open(imgname).convert('RGB')
    input_img = pipe.image_processor.preprocess(img,).to('cuda')
    pipe.upcast_vae()
    posterior = pipe.vae.encode(input_img).latent_dist
    ...
    output.images[0].save(f"/home/ubuntu/tiansheng/Fk-Diffusion-Steering/final_result/test_sdxl_inverse_image_{end_timesteps}.png")
